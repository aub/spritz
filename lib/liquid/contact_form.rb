  ...
  def render(context)
    ...
    context.stack do
      ...
      context['form'] = {
        ...
        'message' => %(<textarea id="contact-message" name="contact[message]" rows="6">#{values['message']}</textarea>),
      ...
      message = context['message'].blank? ? '' : %(<p id="comment-message">#{context['message']}</p>)
