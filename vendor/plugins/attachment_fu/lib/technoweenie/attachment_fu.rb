  ...
  module AttachmentFu # :nodoc:
    ...
    module ActMethods
      ...
      def has_attachment(options = {})
        ...
        unless options[:thumbnails].is_a?(Hash) || options[:thumbnails].is_a?(Symbol)
          raise ArgumentError, ":thumbnails option should be either a hash or a symbol: e.g. :thumbnails => { :foo => '50x50' }"
        end
    ...
    module InstanceMethods
      ...
      protected
        ...
        # Gather the thumbnail options. These can be configurable by passing a symbol
        # to has_attachment that refers to a function you would like it to call to
        # retreive the thumbnail params.
        def evaluate_thumbnail_parameters(params)
          return params if params.is_a?(Hash)
          (params.is_a?(Symbol) && self.respond_to?(params)) ? self.send(params) : {}
        end
        ...
        def after_process_attachment
          if @saved_attachment
            if respond_to?(:process_attachment_with_processing) && thumbnailable? && !attachment_options[:thumbnails].blank? && parent_id.nil?
              ...
              thumbs = evaluate_thumbnail_parameters(attachment_options[:thumbnails])
              thumbs.each { |suffix, size| create_or_update_thumbnail(temp_file, suffix, *size) }
            end
            save_to_storage
            @temp_paths.clear
        ...
        # Determine what the size parameter is and handle it
        # Options are
        #   1. String "100x50"
        #   2. Array  [100,50]
        #   2. Method to return one of the above
        def evaluate_size_parameter(param)
          return param if param.is_a?(String)
          return param if param.is_a?(Array) && param.length > 1
          param = param[0] if param.is_a?(Array)          
          (param.is_a?(Symbol) && self.respond_to?(param)) ? self.send(param) : param
        end

        # Resizes the given processed img object with either the attachment resize options or the thumbnail resize options.
        def resize_image_or_thumbnail!(img)
          if (!respond_to?(:parent_id) || parent_id.nil?) && attachment_options[:resize_to] # parent image
            resize_image(img, evaluate_size_parameter(attachment_options[:resize_to]))
          elsif thumbnail_resize_options # thumbnail
            resize_image(img, evaluate_size_parameter(thumbnail_resize_options)) 
          end
        end
